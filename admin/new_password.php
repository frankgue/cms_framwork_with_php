<?php
    ...
    if (isset($_GET['email']) && !empty($_GET['email']) && isset($_GET['token']) && !empty($_GET['token'])) {
      ...
      $requete = $bdd->prepare("SELECT * FROM utilisateurs WHERE email_utilisateur=:email AND token_utilisateur=:token AND role_utilisateur=:role_utilisateur");
      ...
      if ($nombre != 1) {
        ...
        if (isset($_POST['renew_password'])) {
            ...
            if (empty($_POST['password']) || $_POST['password'] != $_POST['confirm_password']) {
                ...
                $req = $bdd->prepare("UPDATE utilisateurs SET password_utilisateur=:password WHERE email_utilisateur=:email AND role_utilisateur=:role_utilisateur");
