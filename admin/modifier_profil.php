<?php
    ...
    if (isset($_GET['modifier_compte']) && isset($_SESSION['id_utilisateur']) && $_GET['modifier_compte'] == $_SESSION['id_utilisateur']) {
        ...
        if (isset($_POST['modif_profil'])) {
            ...
            $reqUsername = $bdd->prepare("SELECT * FROM utilisateurs WHERE username=:username AND role_utilisateur=:role_utilisateur");
            $reqUsername->bindvalue(':username', $_POST['username']);
            $reqUsername->bindvalue(':role_utilisateur', 'Admin');
            $reqUsername->execute();
            ...
            if ($resultUsername) {
                ...
                $requete2 = $bdd->prepare("UPDATE utilisateurs SET nom_utilisateur=:nom, prenom_utilisateur=:prenom, username=:username, photo_utilisateur=:photo_profil WHERE id_utilisateur=:id_utilisateur AND role_utilisateur=:role_utilisateur");
