<?php
    ...
    if (isset($_SESSION['id_utilisateur'])) {
        ...
        if (isset($_POST['valider_supp_compte'])) {
            $requete = $bdd->prepare("DELETE FROM utilisateurs WHERE id_utilisateur=:id_utilisateur");
                            ...
                            <div class="col-lg-5">
                                <div class="card shadow-lg border-0 rounded-lg mt-5">
                                    ...
                                    <div class="card-footer text-center py-3">
                                        <?php 
                                            if (isset($_GET['supprimer_compte']) && isset($_SESSION['id_utilisateur']) AND $_GET['supprimer_compte'] == $_SESSION['id_utilisateur'] ) {
                                                echo "Voulez-vous vraiment supprimer votre compte?";
                                                echo '<form action="" method="post">
                                                    <div class="d-grid">
                                                        <input type="submit" name="valider_supp_compte" class="btn btn-primary btn-block" value="Oui supprimer mon compte" />
                                                    </div>
                                                </form>';
                                            }
                                        ?>
